...
{
  QString pixfmt;
  pixfmt += static_cast<char> ( id & 0xff );
  pixfmt += static_cast<char> ( ( id >> 8 ) & 0xff );
  pixfmt += static_cast<char> ( ( id >> 16 ) & 0xff );
  pixfmt += static_cast<char> ( ( id >> 24 ) & 0xff );
  return pixfmt;
}
...
bool v4l2::qbuf_user_cap ( int index, void *ptr, int length )
{
  ...
  buf.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
  buf.memory = V4L2_MEMORY_USERPTR;
  buf.m.userptr = reinterpret_cast<unsigned long> ( ptr );
  buf.length = length;
  buf.index = index;
  return qbuf ( buf );
}
